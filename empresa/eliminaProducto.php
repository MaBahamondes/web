...
// 
	...
	$consulta_sql = "SELECT * FROM productos de WHERE nombre_producto ='" . $nombre_producto . "' OR imagen = '" . $imagen . "'";
	   ...
	   if ($resultado_consulta -> num_rows > 0) {
	     echo("<script>alert('Este Producto ya no existe! ')</script>");
	     echo("<script>location.href='https://usuario38.talleresmelipilla.cl/evaluacion_3/vistas_usuario/empresa/pyme.php'</script>");
	     
	     //-----------------------------------------------------------------
    
	   }else{
	       	$consulta = "DELETE INTO `productos` (`id_producto`, `nombre_producto`, `valor_producto`, `resena_producto`, `imagen`) VALUES (NULL, '" . $nombre_producto . "', '" . $valor_producto . "', '" . $resena_producto . "', '" . $imagen . "')";
